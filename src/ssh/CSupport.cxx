...
ChannelSupport::HandleChannelData(std::span<const std::byte> payload)
{
	...
	if (channel.IsEof())
		throw Connection::ProtocolError{"CHANNEL_DATA after CHANNEL_EOF"sv};
...
ChannelSupport::HandleChannelExtendedData(std::span<const std::byte> payload)
{
	...
	if (channel.IsEof())
		throw Connection::ProtocolError{"CHANNEL_EXTENDED_DATA after CHANNEL_EOF"sv};
...
ChannelSupport::HandleChannelEof(std::span<const std::byte> payload)
{
	...
	if (channel.IsEof())
		throw Connection::ProtocolError{"Duplicate CHANNEL_EOF"sv};
