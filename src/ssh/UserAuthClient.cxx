...
UserAuthClient::Start()
{
	...
	connection.SendPacket(MakeServiceRequest("ssh-userauth"sv));
					     ...
					     std::string_view key_algorithm)
{
	...
	PacketSerializer s{MessageNumber::USERAUTH_REQUEST};
	...
	s2.WriteU8(static_cast<uint_least8_t>(MessageNumber::USERAUTH_REQUEST));
...
{
	const auto p = ParseServiceAccept(payload);
...
bool
UserAuthClient::HandlePacket(MessageNumber msg,
			     std::span<const std::byte> payload)
{
	assert(connection.IsEncrypted());

	switch (msg) {
	case MessageNumber::SERVICE_ACCEPT:
	...
	case MessageNumber::REQUEST_SUCCESS:
	case MessageNumber::REQUEST_FAILURE:
